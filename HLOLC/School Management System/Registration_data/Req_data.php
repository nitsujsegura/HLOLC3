...
if (isset($_SESSION)) {
    ...
    $data = 'Email='.$Email.'&Contact_Number='.$Contact_Number.'&Present_address='.$Present_address.'&First_name='.$First_name.'&Middle_name='.$Middle_name.'&Last_name='.$Last_name.'&Age='.$Age.'&Type_of_student='.$Type_of_student.'&Child_birthday='.$Child_birthday.'&gender='.$gender.'&Father_Name='.$Father_Name.'&Mother_name='.$Mother_name.'&grade='.$grade;
    ...
    $sql  = "INSERT INTO
    registration(Email,user_id,Contact_Number, Present_address, First_name, Middle_name, Last_name, Age, Type_of_student, Child_birthday, gender, Father_Name, Mother_name, grade)
    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
if($stmt){  
    ...
    if($last_id){
        ...
        $sql= "UPDATE registration SET user_id = '".$user_id."' WHERE registration = '".$last_id."'";
        $stmt = $conn->prepare($sql);
    }
}
    
$stmt->execute([$Email,$last_id, $Contact_Number, $Present_address, $First_name,$Middle_name,$Last_name, $Age, $Type_of_student, $Child_birthday, $gender, $Father_Name, $Mother_name, $grade]);
